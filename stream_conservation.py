...
if select == 'Donors':
    ...
    if select_don == 'Your story':
        ...
        st.write('In **2020**, you donated ** $' +
                 ...
                 '''** to us. Your kind promoted the environment in Madagascar, and it was highly appreciated! Your tree support ${\color{green}{\t{25}}}$ pounds of species, absorb more than ${\color{red}{\t {48}}}$ pounds of carbon dioxide from the atmosphere each year, and release oxygen in exchange''')
    ...
    if select_don == "Donor's trees":
        fig = px.scatter_mapbox(donor, lat='Latitude', lon='Longitude', hover_name="Donor", hover_data=['Date planted', 'Family', 'Genus', 'Species'],
                                zoom=17, color='Family', title='We keep track of every tree we have planted')
        fig.update_layout(mapbox_style="open-street-map")
        fig.update_layout(width=1000, height=600)


        st.write(fig)
        st.write('Visualization of the location of the trees that each donor donated in Madagascar. This map locates each tree donated as a dot among the matrix of trees.')
        ...
        fig = px.pie(donor_agg, values='Donor', names='Family', labels={'Donor': 'Number of Donors'},
                     title="We have six types of trees")
        fig.update_traces(textposition='outside', textinfo='percent+label', marker_colors=[
                          'lightblue', 'purple', 'orange', 'blue', 'red', 'lightgreen'])
    ...
    if select_don == "Trees planted in other years":
        ...
        trees['Year of planting'] = [
            i[i.find('20'):i.find('20')+4] for i in trees['Year of planting']]
        trees['Year of planting'] = trees['Year of planting'].apply(
            lambda x: int(x) if x != 'n' else np.nan)

        trees_t = pd.concat([trees.groupby(['Latitude1', 'Longitude1']).count()['Year'],
                             trees[trees['Status'] == 'Dead'].groupby(
                                 ['Latitude1', 'Longitude1']).count()['Status'],
                             trees[trees['Status'] == 'Alive'].groupby(
                                 ['Latitude1', 'Longitude1']).count()['Year'],
        ...
        trees_t['Year of planting'] = trees_t['Year of planting'].astype(
            int).astype(str)
        trees_t['Year of planting'][trees_t['Year of planting'] == '0'] = np.nan
        trees_t = trees_t.reset_index()
        trees_t['alive rate'] = round(
            trees_t['Alive']/trees_t['Total trees'], 2)

        st.write('This graph shows the proportion of trees planted in different areas. Trees in most location are all alive. Hovering the mouse over each dot reveals the date of when the tree is planted.')
        ...
        fig = px.pie(trees_agg, values='number of tree', names='Status',
                     labels={'number of tree': 'number of tree'}, title='Satisfying tree survival rate')
        fig.update_traces(textposition='inside', textinfo='percent+label',
                          marker_colors=['green', 'lightgrey'])
        fig.update_layout(showlegend=False, width=1000, title_x=0.5)
        st.write(fig)
...
if select == 'Conservation-360':
    ...
    if select_con == 'natural observation':
        ...
        naturalist_data['taxon_kingdom_name'][pd.isnull(
            naturalist_data['taxon_kingdom_name'])] = 'Other'
        unique_class = list(naturalist_data['taxon_class_name'].unique())
        select_class = st.sidebar.multiselect(
            'Taxon Class', unique_class, unique_class)
        naturalist_data = naturalist_data[naturalist_data['taxon_class_name'].isin(
            select_class)]
        unique_family = list(naturalist_data['taxon_family_name'].unique())
        select_family = st.sidebar.multiselect(
            'Taxon Family', unique_family, unique_family)
        naturalist_data = naturalist_data[naturalist_data['taxon_family_name'].isin(
            select_family)]
        st.write('Natural Observations in this Region')
            
        fig = px.scatter_mapbox(naturalist_data, lat='latitude', lon='longitude', hover_name='common_name',
                                hover_data=[
                                    'scientific_name', 'taxon_family_name', 'taxon_genus_name', 'taxon_species_name'],
                                zoom=10, color='taxon_kingdom_name', title='Planting trees are effective to ecosystem')
        fig.update_layout(mapbox_style="open-street-map")
        ...
        user_input0 = st.text_input(
            "Please input the common name", 'Small-toothed Sportive Lemur')
        st.write('Scientific name of %s' % user_input0+' is '+"**'"+list(
            naturalist_data['scientific_name'][naturalist_data['common_name'] == user_input0])[0]+"'**")
        st.write('**Step2:Copy its name and paste in the below box to show it!!**')
        user_input = st.text_input(
            "Please input the scientific name", 'Lepilemur microdon')
    ...
    if select_con == 'seed collection process':
        ...
        df1.groupby('date').sum()['QuantityOfSeeds (Kg)'][:-1].plot(kind='line', xlabel='',
                                                                    ylabel='QuantityOfSeeds (Kg)', title='Quantity of Seeds Collected Each Year', ax=plt.gca())
        st.write(fig)
    ...
    if select_con == 'transplanting process':
        ...
        every tree's information. Our trees will be planted closer to the forest when they are almost good.''')
        ...
        df2['notes'][df2['Notes'] ==
                     'Regular'] = 'Transplangting with Conservation-360 engagement'
...
if select == 'Madagascar':
    ...
    if select_mdg == 'Income':
        wdbincome = pd.read_csv(
            r'./madagascar/Income per capita WB.csv').iloc[:2, [0]+list(range(4, 22))]
        wdbincome.columns = [
            'country']+[i[i.find('20'):i.find('20')+4] for i in wdbincome.columns[1:]]
        wdbincome = wdbincome.set_index('country').T
        fig = go.Figure(data=[
            go.Scatter(name='United States', x=wdbincome.index,
                       y=wdbincome['United States']),
            go.Scatter(name='Madagascar', x=wdbincome.index,
                       y=wdbincome['Madagascar'])
        ])
        st.markdown("National Income of USA and Madagascar per Capita(USD)")
        fig.update_layout(title='Madagascar is in need for help',
                          title_x=.5, width=1000, xaxis_title='Year')
        st.write(fig)
        st.markdown(read_markdown_file("b6.md"), unsafe_allow_html=True)
        income = pd.read_excel(
            r'./madagascar/Localized (ranomafana) Households Income.xls').iloc[3:9, 1:7]
        income = income.reset_index()
        income.columns = list(income.iloc[0][:-1])+['Yearly Income(USD)']
        income = income.iloc[1:, 1:].sort_values(
            by='Yearly Income(USD)', ascending=False)
        fig = px.bar(income, x='Crops', y='Yearly Income(USD)',
                     title='Residents get the largest profit ratio from growing <b style="color:green">coffee</b>')
        fig.update_traces(marker_color=['red', 'blue', 'blue', 'blue', 'blue'])
        fig.update_layout(title_x=0.5, width=1000)
        st.write(fig)
        st.markdown(read_markdown_file("b9.md"), unsafe_allow_html=True)
    if select_mdg == 'Environment':
        mdg = [pd.read_excel(r'./madagascar/MDG.xlsx', sheet_name=i)
               for i in range(1, 7)]
        ...
        mdg = mdg_sub+mdg_con
        col_name = ['Area']+[i[i.find('20'):i.find('20')+4]
                             for i in mdg_sub[0].columns[1:]]
        for i in mdg:
            i.columns = col_name
        mdg = [i.set_index('Area').T for i in mdg]
        mdg = [pd.concat([mdg[i], mdg[i+3]], axis=1) for i in range(3)]
        mdg_region = {i: pd.concat(
            [mdg[0][i], mdg[1][i], mdg[2][i]], axis=1) for i in mdg[0].columns}
        for i in mdg_region:
            mdg_region[i].columns = ['Hectares of tree cover loss',
                                     'Metric tonnes of aboveground biomass loss', 'Metric tonnes of CO2 emissions']
            mdg_region[i]['Tree cover loss index'] = round(
                100*mdg_region[i]['Hectares of tree cover loss']/mdg_region[i]['Hectares of tree cover loss'][0], 2)
            mdg_region[i]['Biomass loss index'] = round(
                100*mdg_region[i]['Metric tonnes of aboveground biomass loss']/mdg_region[i]['Metric tonnes of aboveground biomass loss'][0], 2)
            mdg_region[i]['CO2 emission index'] = round(
                100*mdg_region[i]['Metric tonnes of CO2 emissions']/mdg_region[i]['Metric tonnes of CO2 emissions'][0], 2)
        region_list = list(mdg_region.keys())
        region = st.selectbox(
            'The Region You Want to Compare with', region_list)

        st.markdown('<div style="text-align: center;">Protection of biomass</div>', unsafe_allow_html=True)
        
        fig = make_subplots(rows=1, cols=2, subplot_titles=[
                            'Fianarantsoa', region])
        for i in mdg_region['Fianarantsoa'].columns[-3:-1]:
            fig.add_trace(
                go.Scatter(x=mdg_region['Fianarantsoa'].index,
                           y=mdg_region['Fianarantsoa'][i], name=i),
                row=1, col=1)

        for i in mdg_region[region].columns[-3:-1]:
            fig.add_trace(
                go.Scatter(x=mdg_region[region].index,
                           y=mdg_region[region][i], name=i),
                row=1, col=2)
        fig.update_layout(title='Comparison of <b style="color:green">Fianarantsoa</b> and <b>'+region, legend=dict(orientation="h", x=0.1, y=-0.1),
